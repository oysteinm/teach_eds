...
This approach aligns with my use of collaborative assignments (see @sec-partner), hands-on coding projects (see @sec-casebnp), and my emphasis on student engagement through mid-term feedback (see @sec-partner). Encouraging students to reflect on their learning processes, share their insights, and challenge each other's ideas not only deepens their understanding but also equips them with critical thinking and communication skills essential for professional success.
...
The simplicity and efficiency of Jupyter have made it an indispensable resource. Students can write, execute, and share their code within a highly intuitive interface. For example, in the case of learning R, they can create and run a basic "Hello Jupyter" script directly in their browser, experiencing the immediate satisfaction of seeing their code in action [@al2022]. 
...
![Hello R Code Example](jupyter2.png)

A simple "Hello Jupyter" example written and executed in Jupyter using the R kernel. This demonstrates how first-year students can begin coding immediately without local installations, focusing on understanding programming concepts rather than troubleshooting software setup.
...
The downloaded .csv file from WISEflow contains candidate numbers, scores, and responses. This file is processed programmatically to generate individual feedback reports.

![Markdown Code for Feedback Reports in RStudio](grade2.png)
The Markdown code in RStudio used to generate personalized PDF reports for each student. This workflow ensures reproducibility and efficiency in the grading process.
